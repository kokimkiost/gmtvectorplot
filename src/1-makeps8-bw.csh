...
echo   100   120   1.000   0.500   0.100   0.500   0.300  21 >> format.in3
echo 4 > format.in4
echo   120   140   1.000   0.800   0.100   0.200   0.300  11 >> format.in4
echo   140   160   1.000   0.800   0.100   0.200   0.300  12 >> format.in4
echo   160   180   1.000   0.800   0.100   0.200   0.300  13 >> format.in4
echo   180   200   1.000   0.800   0.100   0.200   0.300  14 >> format.in4
echo 2 > format.in5 
echo   200   250   1.000   0.000   0.000   1.000   0.300   2 >> format.in5
echo   250   300   1.000   0.000   0.000   1.000   0.300  11 >> format.in5
echo 1 > format.in6 
echo   300   400   1.000   0.000   0.300   1.000   0.300  11 >> format.in6
...
echo 120 140 6.1 >> regend.in
echo 140 160 5.9 >> regend.in
echo 160 180 5.7 >> regend.in
echo 180 200 5.5 >> regend.in
echo 200 250 5.3 >> regend.in
echo 250 300 5.1 >> regend.in
echo 300 400 4.9 >> regend.in

cat regend.in | awk '{print 0.1,$3,$2,0}' > regend.xyuv
vectorplot format.in1 regend.xyuv $scale2 | psxy -R -JX -W1,0 -m -K -O >> $case.ps
vectorplot format.in2 regend.xyuv $scale2 | psxy -R -JX -W1,0 -m -K -O >> $case.ps
vectorplot format.in3 regend.xyuv $scale2 | psxy -R -JX -W1,0 -m -K -O >> $case.ps
vectorplot format.in4 regend.xyuv $scale2 | psxy -R -JX -G0 -m -K -O >> $case.ps
vectorplot format.in5 regend.xyuv $scale2 | psxy -R -JX -W1,0 -m -K -O >> $case.ps
vectorplot format.in6 regend.xyuv $scale2 | psxy -R -JX -G0 -m -K -O >> $case.ps
